...
struct ion_heap_ops {
	...
	int (*custom_ioctl)(struct ion_heap *heap, struct ion_buffer *buffer,
			struct ion_custom_data *custom_data);
	int (*get_heap_attr)(struct ion_heap *heap, unsigned int *attr);
};
...
#define ION_HEAP_FLAG_NONCACHED		BIT(1)
#define ION_HEAP_FLAG_SECURE		BIT(2)
 ...
 * @attribute:	heap attribute from dts
 *

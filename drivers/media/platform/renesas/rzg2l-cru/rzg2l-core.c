				       ...
				       unsigned int notification)
{
	...
	csi_pad = media_pad_remote_pad_first(&group->cru->vdev.entity.pads[0]);
	if (csi_pad)
		return  -EMLINK;
...
static int rzg2l_cru_mc_parse_of_graph(struct rzg2l_cru_dev *cru)
{
	...
	ret = v4l2_async_nf_register(&cru->v4l2_dev,
					   &cru->group->notifier);
	if (ret < 0) {
		cru_err(cru, "Notifier registration failed\n");
 ...
 * On RZ/V2N this may be called by other drivers such as IVC
...
EXPORT_SYMBOL_GPL(rzv2n_cru_pm_get);
...
EXPORT_SYMBOL_GPL(rzv2n_cru_pm_put);

			      ...
			      struct v4l2_async_subdev *asd)
{
	...
	/* RZ/V2N doesn't have IMMUTABLE. You may want to remove the link
...
static int rzg2l_csi2_parse_dt(struct rzg2l_csi2 *priv)
{
	...
	ret = v4l2_async_subdev_nf_register(&priv->subdev,
						  &priv->notifier);
	if (ret)

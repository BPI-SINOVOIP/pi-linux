				   ...
				   const struct ov5640_mode_info **new_mode)
{
	...
	pr_info("%s, code=%d, colorspace=%d, width=%d, heigth=%d, bpp=%d\n", __func__, fmt->code, fmt->colorspace, fmt->width, fmt->height, bpp);
...
static int ov5640_update_pixel_rate(struct ov5640_dev *sensor)
{
	...
	pr_info("%s, num_lanes=%d, bpp=%d\n", __func__, num_lanes, bpp);
	do {
		pixel_rate = ov5640_pixel_rates[pixel_rate_id];
		link_freq = pixel_rate * bpp / (2 * num_lanes);
		pr_info("%s, pixel_rate=%d, link_freq=%lld\n", __func__, pixel_rate, link_freq);

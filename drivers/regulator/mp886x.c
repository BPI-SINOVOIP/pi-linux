...
static int mp8869_set_voltage_sel(struct regulator_dev *rdev, unsigned int sel)
{
	...
	ret = regmap_read(rdev->regmap, rdev->desc->vsel_reg, &reg0);
	if (ret < 0)
		return ret;

	ret = regmap_read(rdev->regmap, MP886X_SYSCNTLREG1, &reg1);
	if (ret < 0)
		return ret;
	reg1 |= MP886X_GO;
	ret = regmap_write(rdev->regmap, MP886X_SYSCNTLREG1, reg1);
	if (ret < 0)
		return ret;

	sel <<= ffs(rdev->desc->vsel_mask) - 1;
	reg0 &= ~rdev->desc->vsel_mask;
	reg0 &= ~MP886X_V_BOOT;
	reg0 |= sel;
	return regmap_write(rdev->regmap, rdev->desc->vsel_reg, reg0);
}
...
static int mp8867_set_voltage_sel(struct regulator_dev *rdev, unsigned int sel)
{
	...
	reg0 &= ~MP886X_V_BOOT;
	reg0 |= sel;
	ret = regmap_write(rdev->regmap, rdev->desc->vsel_reg, reg0);
	if (ret < 0)
		return ret;

...
/*Macro definitions*/
#define DRPAI_DRIVER_VERSION        "1.30 rel.3"
...
static void drpai_shutdown(struct platform_device *pdev);
#endif
...
static void drpai_shutdown(struct platform_device *pdev)
{
    ...
    if(-1 == flag_ret)
    {
        ...
        if(0 != R_DRPAI_ClearHashArea(drpai_shared_exclusion))
        {
        ...
        dcache_clean_inval_poc((unsigned long)drpai_shared_exclusion, (unsigned long)drpai_shared_exclusion + drpai_shared_size);
#else
        __flush_dcache_area(drpai_shared_exclusion, drpai_shared_size);
#endif
...
/* V2L conditional compilation */
    ...
    {
        R_DRPAI_ClearFlagManagementArea(drpai_shared_exclusion);

 ...
 */
    ...
    GTP_DEBUG("Ouuu [%d]: x[0] = 0x%x, x[1] = 0x%x, y[0] = 0x%x, y[1] = 0x%x", sensor_id, config[3], config[4], config[5], config[6]);
#else
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    {
        GTP_GPIO_AS_INT(gtp_int_gpio);
        GTP_INFO("ouuu gtp_int_gpio = %d",gtp_int_gpio);
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
    GTP_DEBUG("INT trigger type:%x, irq num = %d", ts->int_trigger_type,ts->client->irq);
#if 1
...
#if 0
    // disabled by ouuu, beacauseof power on allways
...
static int  goodix_ts_init(void)
{
    ...
    printk(KERN_ERR"ouuu init\n");
    GTP_INFO("GTP driver ret = %d",ret);
    return ret;
}

...
rtw_btc_freerun_check(struct btc_coexist *btc)
{
	...
	btc->btc_get(btc, BTC_GET_U1_AP_NUM, &nScanAPNum);
	RTW_INFO("wl_rssi=%d ScanAPNum=%d\n",wl_rssi,nScanAPNum);

	coex_sta->cnt_wl[BTC_CNT_WL_SCANAP]=nScanAPNum;

	if((wl_rssi > 45) && (coex_sta->bt_a2dp_exist)/*BTC_RSSI_HIGH(coex_dm->wl_rssi_state[0])*/) /* > -55dbm*/
	{
		RTW_INFO("wl_rssi is---> HIGH\n");
		return TRUE;
	}

#if 1 /*v01 ap<=7*/
	if((wl_rssi <= 45) && (coex_sta->bt_a2dp_exist) &&
		(nScanAPNum <= 7)) /* < -55dbm*/
	{
		RTW_INFO("wl_rssi is---> LOW\n");
		return FALSE;
	}
#else /*v01 not care ap*/
	if((wl_rssi <= 45) && (coex_sta->bt_a2dp_exist)) /* < -55dbm*/
	{
		RTW_INFO("wl_rssi is---> LOW\n");
		return FALSE;
	}
...
static void rtw_btc_action_bt_a2dp_hid(struct btc_coexist *btc)
{
	...
	if (btc->board_info.btdm_ant_num == 1) { /* Shared-Ant */
		...
		if(coex_sta->bt_ble_exist && coex_sta->bt_a2dp_exist)
		{
			table_case = 121;
		}
		tdma_case = 113;

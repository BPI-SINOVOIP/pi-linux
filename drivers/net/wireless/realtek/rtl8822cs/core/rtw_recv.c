...
exit:
	...
	if (ret == _SUCCESS)
	{
		struct phydm_phyinfo_struct *p_phy_info = &prxattrib->phy_info;
		PHAL_DATA_TYPE           pHalData =  GET_HAL_DATA(padapter);
		int rf_path, j;
		for(rf_path=0;rf_path<pHalData->NumTotalRFPath;rf_path++)
		{
			padapter->recvpriv.raw_rssi_info.mimo_signal_strength[rf_path]=
...
/* Implement moving average by storing samples in a buffer.
 * As new samples come in, remove the oldest and add the newest
 * to the running sum.
 */

static int cb_moving_average(struct ma_buffer *mb, int8_t samp)
{
	...
	u8 *pphy_status)
{
	...
	{
		padapter->recvpriv.raw_rssi_info.mimo_signal_strength[rf_path]=
			cb_moving_average(&ma_buffers[rf_path],p_phy_info->rx_mimo_signal_strength[rf_path]);
	}
#endif

...
static u8 *load_config(dev_data *dev_entry, int *length)
{
	...
	snprintf(name_last, sizeof(name_last), "realtek/%s", config_name);

	result = request_firmware(&fw, name_last, &udev->dev);
	if (result < 0)
		return NULL;
...
int load_firmware(dev_data * dev_entry, uint8_t ** buff)
{
	...
	snprintf(name_last, sizeof(name_last), "realtek/%s", fw_name);

	ret_val = request_firmware(&fw, name_last, &udev->dev);
	if (ret_val < 0) {

 ...
 * Copyright (C) 2022 Piotr Oniszczuk <piotr.oniszczuk@gmail.com>
 */
...
u32 rpivid_get_num_of_controls(struct rpivid_ctx *ctx, u32 id)
{
	unsigned int i;
	...
	for (i = 0; i < RPIVID_CONTROLS_COUNT; i++) {
		...
		if (hdl->error) {
			v4l2_err(&dev->v4l2_dev,
				...
				v4l2_ctrl_get_name(rpivid_controls[i].cfg.id),
				 hdl->error);

...
static int ion_get_heap_info(struct ion_custom_data *custom_data)
{
	struct ion_device *dev = internal_dev;
	struct ion_heap_info *info;
	struct ion_heap *heap;
	int heap_cnt, ret = 0, num = 0;

	down_read(&dev->lock);
	heap_cnt = dev->heap_cnt;
	info = kcalloc(heap_cnt, sizeof(*info), GFP_KERNEL);
	if (!info) {
		up_read(&dev->lock);
		return -ENOMEM;
	}
	...
	plist_for_each_entry(heap, &dev->heaps, node) {
		strncpy(info[num].name, heap->name, HEAP_INFO_NAME_LEN);
		info[num].name[HEAP_INFO_NAME_LEN - 1] = '\0';
		info[num].type = heap->type;
		info[num].id = heap->id;
		if (heap->heap_data) {
			info[num].base = heap->heap_data->base;
			info[num].size = heap->heap_data->size;
			info[num].attribute = heap->heap_data->attribute[1];
		}
		num++;
		if (num >= heap_cnt)
			break;
	...
	if (copy_to_user((void __user *)custom_data->arg, info,
			sizeof(*info) * heap_cnt)) {
		ret = -EFAULT;
		goto copy_err;
	}
...
static int ion_custom_ioctl(struct ion_custom_data *custom_data)
{
	...
	case ION_BERLIN_GETHI:
		ret = ion_get_heap_info(custom_data);
		break;
	default:
		dma_buf = dma_buf_get(custom_data->share_fd);
		if (!dma_buf || !dma_buf->priv) {
			return -EINVAL;
		}
		...
		heap = buffer->heap;
		if (heap && heap->ops->custom_ioctl) {
			ret = heap->ops->custom_ioctl(heap, buffer,
						custom_data);
		} else {
			ret = -EINVAL;
		}
...
static long ion_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_ABI_VERSION:
		...
		break;
	case ION_IOC_CUSTOM:
		ret = ion_custom_ioctl(&data.custom);
		break;
	default:
		return -ENOTTY;
	}
...
static int ion_debug_heap_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "\n-------------------------------------------------------------------------------\n");
	seq_puts(s, "|   pid  |  process_name  |   tid  |   thread_name  |physical|  size  |kmap|\n");
	seq_puts(s, "---------------------------------------------------------------------------------\n");
	total_size = 0;

	mutex_lock(&heap->buffer_lock);
	list_for_each_entry(buffer, &heap->buffer_list, list) {
		struct sg_table *table = buffer->sg_table;
		struct page *page = sg_page(table->sgl);

		total_size += buffer->size;
		seq_printf(s, "|%8u|%16.s|%8u|%16.s|%8lx|%8zx|%4d|\n",
			   buffer->pid, buffer->task_comm, buffer->tid,
			   buffer->thread_name,
			   (unsigned long int)PFN_PHYS(page_to_pfn(page)),
			   buffer->size, buffer->kmap_cnt);
	}

	mutex_unlock(&heap->buffer_lock);
	seq_puts(s, "----------------------------------------------------\n");
	seq_printf(s, "id %d, used: [0x%08zx], total: [0x%08zx]\n",
		   heap->id, total_size,
		   heap->ops->get_pool_size ? (heap->ops->get_pool_size(heap) << PAGE_SHIFT) : 0);
...
int __ion_device_add_heap(struct ion_heap *heap, struct module *owner)
{
	...
	INIT_LIST_HEAD(&heap->buffer_list);
	mutex_init(&heap->buffer_lock);
	...
	debugfs_create_u64("alloc_bytes_wm",
			   ...
			   heap_root,
			   &heap->alloc_bytes_wm);
	debugfs_create_file(heap->name, 0444,
			    heap_root, heap,
			    &debug_heap_fops);

...
struct dma_buf                  *fw_src_dma_buf;
struct dma_buf                  *fw_dma_buf;
...
phys_addr_t getSecureHeapPaddr(void)
{
	...
	if(ori & 0xFFFF) {
		printk(KERN_INFO "fw_dma_buf phy addr is align to 0xFFFF, change it to %llx\n", (ori+0xFFFF)&(~0xFFFF));
	}
...
PVRSRV_ERROR init_tz(void *pvOSDevice)
{
	...
	fw_dma_buf     = ion_alloc((FW_CODE_SIZE+FW_DATA_SIZE)*2 + 0xFFFF, securePoolID, ION_NONCACHED);
#else
